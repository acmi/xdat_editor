...
class Shortcut implements IOEntity {
    ...
    String state
    int modifiable
    @Type(ShortcutItem.class)
    List<ShortcutItem> shortcutItems = []
    @Type(ShortcutSpecialItem.class)
    List<ShortcutSpecialItem> shortcutSpecialItems = []

    @DefaultIO
    static class ShortcutItem implements IOEntity {
    ...
    static class ShortcutSpecialItem implements IOEntity {
        InputKey key = InputKey.IK_None
        String command
    }

    static enum InputKey {
        IK_None('None'),
