 ...
 */
/*
 * Redefined behavior for Boolean properties.
 * <p>Boolean->BooleanProperty changed to Boolean->ObjectProperty&lt;Boolean&gt;
 *     to support null values.</p>
 *
 * Redefined behavior of property accessors generation.
 *
 * @author acmi
 */
...
public class FXBindableASTTransformation implements ASTTransformation {
    ...
    private static final ClassNode BOOLEAN_OBJECT_PROPERTY_CNODE = OBJECT_PROPERTY_CNODE;
    ...
    static {
        ...
        PROPERTY_TYPE_MAP.put(ClassHelper.Boolean_TYPE, BOOLEAN_OBJECT_PROPERTY_CNODE);
    ...
    public void visit(ASTNode[] nodes, SourceUnit sourceUnit) {
        ...
        if (parent instanceof FieldNode) {
            ...
            if ((modifiers & Modifier.FINAL) != 0) {
                String msg = "@groovyx.javafx.beans.FXBindable cannot annotate a final property.";
    ...
    private void addJavaFXProperty(SourceUnit source, AnnotationNode node, ClassNode declaringClass, FieldNode field) {
        ...
        for (PropertyNode propertyNode : declaringClass.getProperties()) {
            if (propertyNode.getName().equals(fieldName)) {
                if (field.isStatic()) {
                    String message = "@groovyx.javafx.beans.FXBindable cannot annotate a static property.";
        ...
        String message = "@groovyx.javafx.beans.FXBindable must be on a property, not a field. Try removing the private, " +
    ...
    private void createPropertyGetterSetter(ClassNode classNode, PropertyNode originalProp) {
        ...
        for (AnnotationNode annotation : originalProp.getField().getAnnotations()) {
            ...
            if (isTargetAllowed(target, ElementType.FIELD)) {
    ...
    private void createPropertyAccessor(ClassNode classNode, PropertyNode fxProperty, FieldNode fxFieldShortName,
            ...
            if (fxType.getTypeClass() == SIMPLE_LIST_PROPERTY_CNODE.getTypeClass()) {
                if (initExp != null) {
                    if (initExp instanceof ListExpression ||
                            (initExp instanceof CastExpression &&
                                    (initExp.getType().equals(LIST_TYPE) ||
                                            initExp.getType().declaresInterface(LIST_TYPE))) ||
                ...
                if (initExp != null) {
                    if (initExp instanceof MapExpression ||
                            (initExp instanceof CastExpression &&
                                    (initExp.getType().equals(MAP_TYPE) ||
                                            initExp.getType().declaresInterface(MAP_TYPE))) ||
                            (initExp instanceof ConstructorCallExpression &&
                                    (initExp.getType().equals(MAP_TYPE) ||
                                            initExp.getType().declaresInterface(MAP_TYPE)))
                            ) {
                ...
                if (initExp != null) {
                    if ((initExp instanceof CastExpression &&
                            (initExp.getType().equals(SET_TYPE) ||
                                    initExp.getType().declaresInterface(SET_TYPE))) ||
                            (initExp instanceof ConstructorCallExpression &&
                                    (initExp.getType().equals(SET_TYPE) ||
                                            initExp.getType().declaresInterface(SET_TYPE)))
                            ) {
        ...
        String javaFXPropertyFunction = getFXPropertyGetterName(fxProperty);
        MethodNode accessor = new MethodNode(javaFXPropertyFunction, fxProperty.getModifiers(), fxProperty.getType(),
